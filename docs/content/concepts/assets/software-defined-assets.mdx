...
| <PyObject object="SourceAsset" />     | A class that describes an asset, but doesn't define how to compute it. <PyObject object="SourceAsset" />s are used to represent assets that other assets depend on, but can't be materialized themselves. |
...
from dagster import build_assets_job, asset
...
all_assets = build_assets_job([upstream, downstream_1])
```
